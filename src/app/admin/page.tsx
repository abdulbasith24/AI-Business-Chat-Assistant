  ...
  return (
    <div className="group flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-5 shadow-sm hover:border-indigo-200 hover:shadow-md transition-all duration-200">
      <div className="space-y-3">
        ...
        {/* Label (Normalized regular lowercase text-xs muted label) */}
        ...
        {/* Description (Updated to text-sm body description) */}
        <p className="text-sm text-slate-500 leading-relaxed">
          {description}
        </p>
      </div>

      {/* CTA (Minimum text-sm sizing) */}
      <div className="mt-5 pt-4 border-t border-slate-100">
        ...
        >
          {linkLabel}
          <ChevronRightIcon className="w-3.5 h-3.5 transition-transform group-hover/link:translate-x-0.5" />
        </Link>
      </div>
    </div>
  );
...
export default function AdminDashboardOverview() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Quick actions row ────────────────────────────────────────────── */}
      <div>
        {/* Adjusted to section-eyebrow standard: text-xs, font-medium, uppercase, tracking-wide */}
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        ...
        <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          {/* CTA Link set to minimum text-sm layout */}
          <Link
            href="/admin/test-chat"
            className="flex-shrink-0 inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-500 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2"
          >
